...
nvp6324_find_mode(struct nvp6324 *self, enum nvp6324_fps fps,
...
static int nvp6324_try_fmt_internal(struct nvp6324 *nvp6324,
...
static inline void nvp6324_pattern_enable(struct nvp6324 *self)
{
	struct reg_pack reg_pack;
	...
	nvp6324_transfer_regs(self, &reg_pack, 0, 4);
}

static inline void nvp6324_manual_set(struct nvp6324 *self)
{
	int i;
	u8 reg;

	nvp6324_set_reg_bank(self, 0x01);
	nvp6324_write_reg(self, 0x7c, 0x0);

	/* detect */
	nvp6324_set_reg_bank(self, 0x13);
	for (i = 0; i < 4; i++) {
		reg = nvp6324_read_reg(self, 0x30);
		reg &= (~(1 << (i + 4)) & (~(1 << i)));
		nvp6324_write_reg(self, 0x30, reg);

		reg = nvp6324_read_reg(self, 0x31);
		reg &= (~(1 << (i + 4)) & (~(1 << i)));
		nvp6324_write_reg(self, 0x31, reg);

		reg = nvp6324_read_reg(self, 0x32);
		reg &= (~(1 << i));
		nvp6324_write_reg(self, 0x32, reg);
	}
}

static void nvp6324_hardware_init(struct nvp6324 *self)
{
	struct reg_pack reg_pack;
	enum nvp6324_analog_input input = self->analog_input;

	nvp6324_mipi_init(self);

	/* configure differ or single input */
	memcpy(&reg_pack, &nvp6324_input_mode_data[input].reg_pack, sizeof(reg_pack));
	nvp6324_transfer_regs(self, &reg_pack, 0, 4);

	nvp6324_manual_set(self);
	...
	memcpy(&reg_pack, &self->current_mode->reg_pack, sizeof(reg_pack));
	nvp6324_transfer_regs(self, &reg_pack, 0, 4);

	nvp6324_video_eq_set(self);

	nvp6324_mipi_fmt_set(self);
	nvp6324_arb_init(self);
	nvp6324_pattern_enable(self);
}

static void nvp6324_hardware_preinit(struct nvp6324 *self)
{
	int i = 0;

	//Pad Control Setting
	nvp6324_set_reg_bank(self, 0x04);
	for (i = 0; i < 36; i++) {
		nvp6324_write_reg(self, 0xa0 + i, 0x24);
	}

	// Clock Delay Setting
	nvp6324_set_reg_bank(self, 0x01);
	for (i = 0; i < 4; i++) {
		nvp6324_write_reg(self, 0xcc + i, 0x64);
	}

	// MIPI_V_REG_OFF
	nvp6324_set_reg_bank(self, 0x21);
	nvp6324_write_reg(self, 0x07, 0x80);
	nvp6324_write_reg(self, 0x07, 0x00);

	// AGC_OFF  08.31
	nvp6324_set_reg_bank(self, 0x0a);
	nvp6324_write_reg(self, 0x77, 0x8F);
	nvp6324_write_reg(self, 0xF7, 0x8F);
	nvp6324_write_reg(self, 0xff, 0x0B);
	nvp6324_write_reg(self, 0x77, 0x8F);
	nvp6324_write_reg(self, 0xF7, 0x8F);
}

inline enum nvp6324_mode nvp6324_video_current_mode(struct nvp6324 *self)
{
...
int nvp6324_video_init(struct nvp6324 *self)
{
	...
	self->analog_input = nvp6324_input_single;
	self->phy_mclks = nvp6324_756mhz;
	 ...
	 ****************************************/
	self->streamcap.capability = V4L2_MODE_HIGHQUALITY | V4L2_CAP_TIMEPERFRAME;

	self->streamcap.timeperframe.numerator = 1;
	self->streamcap.timeperframe.denominator =
		nvp6324_framerates[nvp6324_fps_25];

	self->current_fps = nvp6324_fps_25;
	self->current_mode =
		&nvp6324_mode_info_data[nvp6324_fps_25][nvp6324_mode_720p];

	self->format.code = MEDIA_BUS_FMT_UYVY8_2X8;
	self->format.width = self->current_mode->width;
	self->format.height = self->current_mode->height;
	self->format.colorspace = V4L2_COLORSPACE_DEFAULT;
	self->format.reserved[0] = 756;
	self->format.field = V4L2_FIELD_NONE;
	...
	sd = &self->v4l2_sd;
	v4l2_i2c_subdev_init(sd, self->i2c_client, &nvp6324_subdev_ops);

	sd->flags = V4L2_SUBDEV_FL_HAS_DEVNODE;
	sd->entity.function = MEDIA_ENT_F_IF_VID_DECODER;
	self->pads[MIPI_CSI2_SENS_VC0_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	self->pads[MIPI_CSI2_SENS_VC1_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	self->pads[MIPI_CSI2_SENS_VC2_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	self->pads[MIPI_CSI2_SENS_VC3_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
