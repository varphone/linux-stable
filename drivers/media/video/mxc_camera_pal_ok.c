...
static void mxc_cam_enc_callback(u32 mask, struct mxc_camera_dev *mxc_cam)
{
	...
	//add by xym
	struct csi_enc_data *enc_data = mxc_cam_to_enc_data(mxc_cam);
	//add by xym

//	dev_err(mxc_cam->icd->dev.parent, "callback, active DMA 0x%08x\n",
...
//edit by xym
	...
	if (list_empty(&mxc_cam->capture)) {
		mxc_cam->active = NULL;
		ret = mxc_cam->enc_ops->enc_update_eba(mxc_cam->ipu, mxc_cam->csi, enc_data->dummy_frame.buffer.m.offset, &mxc_cam->csi_buf_num);
		if(ret) {
			printk(KERN_ERR "%s:%d update_eba failed!", __func__, __LINE__);
		}
	}
	else {
		mxc_cam->active = list_entry(mxc_cam->capture.next, struct mxc_camera_buffer, queue);
		ret = mxc_cam->enc_ops->enc_update_eba(mxc_cam->ipu, mxc_cam->csi, sg_dma_address(&(mxc_cam->active->sg)), &mxc_cam->csi_buf_num);
		if(ret)
			printk(KERN_ERR "%s:%d update_eba failed!", __func__, __LINE__);
	}
	spin_unlock(&mxc_cam->lock);
				...
				struct soc_camera_device *icd)
{
    ...
    if(mxc_cam->csi == 1)
    	csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR1120_PROGRESSIVE_SDR; //IPU_CSI_CLK_MODE_GATED_CLK;
    else
        csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR656_INTERLACED;
	csi_param.ext_vsync = 0;  //1:外同步 0：内同步
...
//		csi_param.data_width = IPU_CSI_DATA_WIDTH_16;
    ...
    if(mxc_cam->csi == 0) 
    	ipu_csi_init_interface(mxc_cam->ipu, 720, 625, IPU_PIX_FMT_UYVY, csi_param);
    else
        ipu_csi_init_interface(mxc_cam->ipu, 1280, 1024, IPU_PIX_FMT_YUYV, csi_param);
			       ...
			       unsigned char buswidth, unsigned long *flags)
{
	...
	*flags = SOCAM_MASTER |
	 ...
	 */
	if (mxc_cam->platform_flags & MXC_CAMERA_DATAWIDTH_16)	
		*flags |=SOCAM_DATAWIDTH_16 | SOCAM_DATAWIDTH_15 | SOCAM_DATAWIDTH_10 |
...
static int mxc_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt)
{
	...
	dev_err(dev, "requested bus width %d bit: %d\n", buswidth, ret); // dev_dbg
	...
	if (common_flags & SOCAM_DATAWIDTH_16)
		common_flags = (common_flags & ~SOCAM_DATAWIDTH_MASK) |
			SOCAM_DATAWIDTH_16;    
	else if (common_flags & SOCAM_DATAWIDTH_15)
		common_flags = (common_flags & ~SOCAM_DATAWIDTH_MASK) |
			SOCAM_DATAWIDTH_15;
	...
	if (ret < 0) {
		dev_err(dev, "camera set_bus_param(%lx) returned %d\n",
			common_flags, ret);
		return ret;
	}
    ...
    if(mxc_cam->csi == 1)
        ...
        csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR656_INTERLACED;
	csi_param.ext_vsync = 0;   //1：外同步模式   0：内同步模式
	csi_param.Vsync_pol = 0;
	csi_param.Hsync_pol = 0;
	csi_param.pixclk_pol = 0;
	csi_param.data_pol = 0;
    ...
    dev_err(dev, "last bits_per_sample: %u, %dx%d\n", xlate->host_fmt->bits_per_sample, icd->user_width, icd->user_height);
#if 0
    printk("csi_param.data_width=%d\n",	csi_param.data_width);
   	printk("csi_param.clk_mode=%d\n",csi_param.clk_mode);
	printk("csi_param.ext_vsync=%d\n",csi_param.ext_vsync);
	printk("csi_param.Vsync_pol=%d\n",csi_param.Vsync_pol);
	printk("csi_param.Hsync_pol=%d\n",csi_param.Hsync_pol);
	printk("csi_param.pixclk_pol=%d\n",csi_param.pixclk_pol);
	printk("csi_param.data_pol=%d\n",csi_param.data_pol);
	printk("csi_param.sens_clksrc=%d\n",csi_param.sens_clksrc);
	printk("csi_param.pack_tight=%d\n",csi_param.pack_tight);
	printk("csi_param.force_eof=%d\n",csi_param.force_eof);
	printk("csi_param.data_en_pol=%d\n",csi_param.data_en_pol);
	printk("csi_param.data_fmt=%d\n",csi_param.data_fmt);
	printk("csi_param.csi=%d\n",csi_param.csi);
	printk("csi_param.mclk=%d\n",csi_param.mclk);
#endif


    if(mxc_cam->csi ==0) 
    	ipu_csi_init_interface(mxc_cam->ipu, 720, 625,  fourcc_to_ipu_pixfmt(xlate->host_fmt->fourcc), csi_param);
    
    else 
    	ipu_csi_init_interface(mxc_cam->ipu, icd->user_width, icd->user_height,  fourcc_to_ipu_pixfmt(xlate->host_fmt->fourcc), csi_param);
...
static int __devinit mxc_camera_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "Probing mxc_camera...\n");
