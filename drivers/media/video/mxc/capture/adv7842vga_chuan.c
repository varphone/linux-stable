...
static video_fmt_t video_fmts[] = {
	...
	{			/*! NTSC */
	 ...
	 .active_width = 3840,	/* ACT_FRM_WIDTH plus 1 */
	 .active_height = 720,	/* ACT_FRM_WIDTH plus 1 */
	 },
	{			/*! Unlocked standard */
	 .v4l2_id = V4L2_STD_ALL,
	 .name = "Autodetect",
...
static int adv7842vga_register_clients(struct i2c_client *client)
{
	...
	data->i2c_sdp_io = adv7842vga_dummy_client(client, "sdp_io", 0x94, 0xf2);
	data->i2c_avlink = adv7842vga_dummy_client(client, "avlink", 0x84, 0xf3);
	data->i2c_cec = adv7842vga_dummy_client(client, "cec", 0x80, 0xf4);
	data->i2c_infoframe = adv7842vga_dummy_client(client, "infoframe", 0x7c, 0xf5);
...
static int ioctl_g_chip_ident(struct v4l2_int_device *s, int *id)
{
	...
	strcpy(((struct v4l2_dbg_chip_ident *)id)->match.name, "adv7842vga_decoder");
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	struct i2c_client *client = sensor->sen.i2c_client;
/*
40 00 16 ; VID_STD=010110b for UXGA60 // 16->17 
...
//40 16 c6 40 17 c2 40 AB 4a 40 AC 30
	io_write(client, 0x16, 0xc6);
	io_write(client, 0x17, 0xc2);
	io_write(client, 0xab, 0x4a);
	io_write(client, 0xac, 0x30);
    io_write(client, 0x00, 0x17);
...
//	gpio_sensor_active();
	...
	if (rev != 0x2012) {
		dev_err(&adv7842_vga.sen.i2c_client->dev, "not an adv7842vga on address 0x%x (rev=0x%04x)\n",  client->addr, rev);
		return -ENODEV;
	}
