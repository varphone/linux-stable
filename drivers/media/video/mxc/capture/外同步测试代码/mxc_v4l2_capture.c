...
static video_fmt_t video_fmts2[] = {
	{			/*! NTSC */
	 .v4l2_id = V4L2_STD_NTSC,
	 .name = "NTSC",
	 .raw_width = 3840,		/* SENS_FRM_WIDTH */
	 .raw_height = 720,		/* SENS_FRM_HEIGHT */
	 .active_width = 3840,		/* ACT_FRM_WIDTH */
	 .active_height = 720,		/* ACT_FRM_HEIGHT */
	 .active_top = 0,
	 .active_left = 0,
	 },
	{			/*! (B, G, H, I, N) PAL */
...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	printk("csi_param.ext_vsync = %d, csi_param.pixclk_pol=%d\n",csi_param.ext_vsync, csi_param.pixclk_pol);
	/* if the capturemode changed, the size bounds will have changed. */
	cam_fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	vidioc_int_g_fmt_cap(cam->sensor, &cam_fmt);
	pr_debug("   g_fmt_cap returns widthxheight of input as %d x %d\n",	cam_fmt.fmt.pix.width, cam_fmt.fmt.pix.height);
//	pr_err("cam_fmt.fmt.pix.pixelformat = %d\n",cam_fmt.fmt.pix.pixelformat);
...
static int mxc_v4l2_s_std(cam_data *cam, v4l2_std_id e)
{
    ...
    if(cam->csi ==0) {
	    ...
	    cam->crop_current.height = video_fmts2[video_index].active_height;
	...
	if (cam->device_type == 1) {
		...
		if (cam->standard_autodetect) {
			if (cam->standard.id != tv_fmt.fmt.pix.pixelformat) {
				pr_debug("MVC: mxc_v4l2_g_std: Changing standard\n");
