...
/*!
 * @file ipu_csi_rot_pp_enc.c
 *
 * @brief CSI with Rotation Use case for video capture
 *
 * @ingroup IPU
 */
...
/*!
 * csi Rotation input done callback function
 */
static irqreturn_t csi_rot_pp_enc_rot_in_callback(int irq, void *dev_id)
{
	cam_data *cam = (cam_data *) dev_id;

	if (cam->csi_irq_pending) {
		ipu_select_buffer(cam->ipu, MEM_ROT_PP_MEM, IPU_INPUT_BUFFER,
				  cam->rot_enc_buf_num);
		cam->csi_irq_pending = 0;
	}
...
/*!
 * csi output done callback function
 */
static irqreturn_t csi_rot_pp_enc_csi_out_callback(int irq, void *dev_id)
{
	cam_data *cam = (cam_data *) dev_id;

	cam->rot_enc_buf_num = cam->csi_mem_buf_num;
	cam->csi_mem_buf_num = (~cam->csi_mem_buf_num) & 0x01;
	ipu_select_buffer(cam->ipu, CSI_MEM, IPU_OUTPUT_BUFFER,
			  cam->csi_mem_buf_num);
	cam->csi_irq_pending = 1;
 ...
 */
static irqreturn_t csi_rot_pp_enc_callback(int irq, void *dev_id)
{
	cam_data *cam = (cam_data *) dev_id;
...
/*!
 * CSI with Rotation ENC enable channel setup function
 *
 * @param cam       struct cam_data * mxc capture instance
 *
 * @return  status
 */
static int csi_rot_pp_enc_setup(cam_data *cam)
{
	...
	CAMERA_TRACE("In csi_rot_pp_enc_setup\n");
	if (!cam) {
	...
	if (cam->rotation == IPU_ROTATE_NONE) {
		...
		cam->rot_enc_buf_size[0] = PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage);
		cam->rot_enc_bufs_vaddr[0] =
		...
		cam->rot_enc_buf_size[1] = PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage);
		cam->rot_enc_bufs_vaddr[1] =
 ...
 * @return  status
 */
static int csi_rot_pp_enc_eba_update(struct ipu_soc *ipu, dma_addr_t eba, int *buffer_num, void *private)
{
	...
	if (cam->rotation == IPU_ROTATE_NONE) {
		...
		if (err != 0) {
			...
			err = ipu_update_channel_buffer(ipu, MEM_ROT_PP_MEM, IPU_OUTPUT_BUFFER,
							*buffer_num, eba);
			if (err != 0) {
				pr_err("ERROR: v4l2 capture: fail to update "
				       "buf%d\n", *buffer_num);
 ...
 * @return  status
 */
static int csi_rot_pp_enc_enabling_tasks(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
	CAMERA_TRACE("IPU:In csi_rot_pp_enc_enabling_tasks\n");
	...
	if (cam->rotation == IPU_ROTATE_NONE) {
		...
		err = ipu_request_irq(cam->ipu, IPU_IRQ_CSI0_OUT_EOF,
				      csi_rot_pp_enc_callback, 0, "Mxc Camera", cam);
	} else {
		ipu_clear_irq(cam->ipu, IPU_IRQ_PP_ROT_IN_EOF);
		err = ipu_request_irq(cam->ipu, IPU_IRQ_PP_ROT_IN_EOF,
				      csi_rot_pp_enc_rot_in_callback, 0, "Mxc Camera (ROT IN)", cam);
		...
		ipu_clear_irq(cam->ipu, IPU_IRQ_CSI0_OUT_EOF);
		err = ipu_request_irq(cam->ipu, IPU_IRQ_CSI0_OUT_EOF,
				      csi_rot_pp_enc_csi_out_callback, 0, "Mxc Camera (CSI OUT)", cam);
	}
	...
	err = csi_rot_pp_enc_setup(cam);
	if (err != 0) {
		printk(KERN_ERR "csi_rot_pp_enc_setup %d\n", err);
		return err;
	}
 ...
 * @return  int
 */
static int csi_rot_pp_enc_disabling_tasks(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
#ifdef CONFIG_MXC_MIPI_CSI2
 ...
 * @return  status
 */
static int csi_rot_pp_enc_enable_csi(void *private)
{
	cam_data *cam = (cam_data *) private;
 ...
 * @return  status
 */
static int csi_rot_pp_enc_disable_csi(void *private)
{
	cam_data *cam = (cam_data *) private;
...
int csi_rot_pp_enc_select(void *private)
{
	...
	if (cam) {
		cam->enc_update_eba = csi_rot_pp_enc_eba_update;
		cam->enc_enable = csi_rot_pp_enc_enabling_tasks;
		cam->enc_disable = csi_rot_pp_enc_disabling_tasks;
		cam->enc_enable_csi = csi_rot_pp_enc_enable_csi;
		cam->enc_disable_csi = csi_rot_pp_enc_disable_csi;
	} else {
		err = -EIO;
	}
 ...
 */
__init int csi_rot_pp_enc_init(void)
{
	return 0;
}
 ...
 *
 */
void __exit csi_rot_pp_enc_exit(void)
{
}

module_init(csi_rot_pp_enc_init);
module_exit(csi_rot_pp_enc_exit);

EXPORT_SYMBOL(csi_rot_pp_enc_select);
EXPORT_SYMBOL(csi_rot_pp_enc_deselect);

MODULE_AUTHOR("Freescale Semiconductor, Inc.");
MODULE_DESCRIPTION("CSI with Rotation ENC Driver");
MODULE_LICENSE("GPL");
