...
static struct i2c_client *adv7842camera_dummy_client(struct i2c_client *client,
...
static int adv7842camera_register_clients(struct i2c_client *client)
{
	...
	data->i2c_sdp_io =
		adv7842camera_dummy_client(client, "sdp_io", 0x94, 0xf2);
	data->i2c_avlink =
		adv7842camera_dummy_client(client, "avlink", 0x84, 0xf3);
	data->i2c_cec = adv7842camera_dummy_client(client, "cec", 0x80, 0xf4);
	data->i2c_infoframe =
		adv7842camera_dummy_client(client, "infoframe", 0x7c, 0xf5);
			       ...
			       const struct i2c_device_id *id)
{
	...
	dev_err(&adv7842_data.sen.i2c_client->dev,
		"%s:adv7842 probe i2c address is 0x%02X\n", __func__,
		adv7842_data.sen.i2c_client->addr);

	adv7842camera_reset(client); /* reset adv7842 */
	...
	if (rev != 0x2012) {
		dev_err(&adv7842_data.sen.i2c_client->dev,
			"not an adv7842camera on address 0x%x (rev=0x%04x)\n",
			client->addr, rev);
		return -ENODEV;
	}
	dev_err(&adv7842_data.sen.i2c_client->dev,
		"find adv7842camera on address 0x%x (rev=0x%04x)\n",
		client->addr, rev);
