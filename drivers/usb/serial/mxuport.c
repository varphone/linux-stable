					...
					void *dest, size_t size)
{
	...
	put_unaligned_be16(port->number, buf);
					  ...
					  char *data, int size)
{
	...
	if (!port->port.console || !port->sysrq) {
		tty_insert_flip_string(port->port.tty, data, size);
	} else {
		for (i = 0; i < size; i++, data++) {
			if (!usb_serial_handle_sysrq_char(port, *data))
				tty_insert_flip_char(port->port.tty, *data,
						     TTY_NORMAL);
		}
	}
			 ...
			 const struct usb_device_id *id)
{
	...
	if (err) {
		dev_warn(&serial->interface->dev, "Firmware %s not found, error %d\n",
			 buf, err);
...
static int mxuport_attach(struct usb_serial *serial)
{
	 ...
	 */
	err = usb_serial_generic_submit_read_urb(port0, GFP_KERNEL);
	if (err)
		return err;

	err = usb_serial_generic_submit_read_urb(port1, GFP_KERNEL);
	if (err) {
...
static int mxuport_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	err = mxuport_send_ctrl_urb(serial, RQ_VENDOR_SET_RX_HOST_EN,
				    1, port->number);
	if (err) {
		dev_err(&port->dev, "RQ_VENDOR_SET_RX_HOST_EN error: %d\n", err);
		return err;
	}
	...
	err = mxuport_send_ctrl_urb(serial, RQ_VENDOR_SET_OPEN,
				    1, port->number);
	if (err) {
		dev_err(&port->dev, "RQ_VENDOR_SET_OPEN error: %d\n", err);
		mxuport_send_ctrl_urb(serial, RQ_VENDOR_SET_RX_HOST_EN,
				      0, port->number);
		return err;
	}
...
static int mxuport_resume(struct usb_serial *serial)
{
	...
	for (i = 0; i < 2; i++) {
		...
		r = usb_serial_generic_submit_read_urb(port, GFP_NOIO);
		if (r < 0)
			c++;
	}
...
static int __init mxuport_init(void)
{
	...
	retval = usb_serial_register(&mxuport_device);
	if (retval)
		goto failed_port_device_register;
	
	printk(KERN_INFO "mxuport: usb serial device registered.");

	retval = usb_register(&mxuport_driver);
	if (retval == 0) {
		printk(KERN_INFO "mxuport: usb serial driver registered.");
		return 0;
	}

	usb_serial_deregister(&mxuport_device);
