...
{
	/* 32 micro frames in the future */
	u16 fframe_off = 32;
	u16 cframe =  __dwc3_gadget_get_frame(dep->dwc);
	u16 fframe = cframe + fframe_off;
	u16 extraf = dep->frame_number & DWC3_EVENT_PRAM_SOFFN_EXTRA_MASK;
	...
	if ((fframe > DWC3_DSTS_MAX_SOFFN) ||
		(((dep->frame_number & DWC3_EVENT_PRAM_SOFFN_MASK) +
		fframe_off) > DWC3_EVENT_PRAM_MAX_SOFFN)) {
		extraf += DWC3_EVENT_PRAM_SOFFN_EXTRA(1);
		fframe %= DWC3_DSTS_MAX_SOFFN;
	}

	dep->frame_number = extraf | fframe;
