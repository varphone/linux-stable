	...
	uint32_t pixel_fmt, ipu_csi_signal_cfg_t cfg_param)
{
	...
	case IPU_PIX_FMT_GENERIC_16:
		printk("data is IPU_PIX_FMT_GENERIC_16\n");
	...
	dev_err(ipu->dev, "csi=%d, CSI_SENS_CONF = 0x%08X\n",csi,  //by lwx   dev_dbg
		ipu_csi_read(ipu, csi, CSI_SENS_CONF));   

	dev_err(ipu->dev, "csi=%d,CSI_ACT_FRM_SIZE=0X%08X\n", csi,
