...
{
	struct input_dev *dev = pm->TouchDev;
...
static void pm_parse_3000(struct pm *pm)
{
	...
	if (pm->packetsize == ++pm->idx) {
		if (pm_checkpacket(pm->data)) {
			if ((pm->data[0] & 0x0F) == 0x0A) {
				if (pm->data[0] == 0x8A) {
					input_report_key(pm->KbdDev, g_AbnormalKey, 1);
					printk ("[PENMOUNT] Enter Abnormal State !!!\n");
				} else {
					input_report_key(pm->KbdDev, g_AbnormalKey, 0);
					printk ("[PENMOUNT] Leave Abnormal State !!!\n");
				}
				pm_mtevent(pm, pm->KbdDev);
			} else if ((pm->data[0] & 0xce) == 0x40) {
