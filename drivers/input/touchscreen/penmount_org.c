		...
		unsigned char data, unsigned int flags)
{
	...
	if (pm->data[0] & 0x80) {
		if (PM_MAX_LENGTH == ++pm->idx) {
			...
			input_report_abs(dev, ABS_PRESSURE, !!(pm->data[0] & 0x40));   /* by lwx */
			input_sync(dev);
			pm->idx = 0;
		}
	}
...
#else
	setbit(EV_KEY, pm->dev->evbit);
	setbit(EV_ABS, pm->dev->evbit);

	setbit(BTN_TOUCH, pm->dev->keybit);
#endif

    input_set_abs_params(pm->dev, ABS_X, 0, 0x3ff, 0, 0);
    input_set_abs_params(pm->dev, ABS_Y, 0, 0x3ff, 0, 0);
    input_set_abs_params(pm->dev, ABS_PRESSURE, 0, 1, 0, 0); /*add my lwx */
