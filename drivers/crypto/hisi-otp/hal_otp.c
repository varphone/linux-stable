...
HI_S32 HAL_Efuse_OTP_Init(HI_VOID)
{
    ...
    u32SysAddr = ioremap_nocache(REG_SYS_OTP_CLK_ADDR_PHY, 0x100);
    ...
    g_u32EfuseOtpRegBase = ioremap_nocache(OTP_REG_BASE_ADDR_PHY, 0x100);
...
/* CRC16 */
static unsigned short CalculateCRC16 (const unsigned char* data_array_ptr, unsigned long data_array_length)
{
...
static HI_S32 HAL_OTP_WaitFree(HI_VOID)
{
...
static HI_S32 HAL_OTP_SetMode(OTP_USER_WORK_MODE_E enOtpMode)
{
...
static HI_U32 HAL_Get_OTP_Lock_status_byType(OTP_LOCK_STA_TYPE_E enLockStatusType)
{
    HI_U32 Regddr = 0;
    HI_U32 RegValue = 0;

    Regddr = (unsigned int)((enLockStatusType == OTP_USER_LOCK_STA0_TYPE)?OTP_USER_LOCK_STA0:OTP_USER_LOCK_STA1);
...
static HI_VOID HAL_CHOOSE_OTP_key(OTP_USER_KEY_INDEX_E enWhichKey)
{
    HI_U32 RegValue = 0;
...
/* write data to otp */
static HI_S32 HAL_Write_key(OTP_USER_KEY_LENGTH_E enKeyLength,HI_U32 *KeyDataBuf,HI_U32 UseCRC)
{
    HI_U32 crc16 = 0;
...
static HI_S32 HAL_Get_OTP_Lock_status(OTP_LOCK_STA_TYPE_E enLockStatusType,HI_U32 *status)
{
...
static HI_S32 Is_Locked(OTP_USER_KEY_INDEX_E enWhichKey,HI_U32 lock_sta)
{
...
EXPORT_SYMBOL(HAL_Key_CRC_CheckOut);
