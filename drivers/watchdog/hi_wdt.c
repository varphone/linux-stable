...
MODULE_PARM_DESC(default_margin,
                 "Watchdog default_margin in seconds. (0<default_margin<80, default="
                 __MODULE_STRING(HIDOG_TIMER_MARGIN) ")");
...
static inline void hidog_start(void)
{
	...
	printk(KERN_INFO HIDOG_PFX "started.\n");
}
...
static inline void hidog_stop(void)
{
	...
	printk(KERN_INFO HIDOG_PFX "stopped.\n");
}
...
static ssize_t hidog_write(struct file* file, const char __user* data,
...
static char banner[] __initdata = KERN_INFO
                                  "Hisilicon Watchdog Timer: 0.01 initialized. default_margin=%d sec (nowayout= %d, nodeamon= %d)\n";
...
static int __init hidog_init(void)
{
	...
	if (!nodeamon) {
		...
		p_dog = kthread_create(hidog_deamon, NULL, "hi_wdt");
		if (IS_ERR(p_dog) < 0) {
...
{
	int ret = 0;
	reg_wdt_base_va = ioremap_nocache((unsigned long)HIWDT_BASE,
