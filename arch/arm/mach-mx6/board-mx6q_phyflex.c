...
//#define MX6_PHYFLEX_CAP_TCH_INT0	IMX_GPIO_NR(5, 8)  //by lwx
//#define MX6_PHYFLEX_CAP_TCH_INT1	IMX_GPIO_NR(5, 7)  // by lwx
#define MX6_PHYFLEX_POWER_OFF	    IMX_GPIO_NR(5, 8)  // 单片机向IMX6发送断电中断
#define MX6_PHYFLEX_POWER_SND_OFF   IMX_GPIO_NR(5, 7)  // IMX6向单片机发送断电完成
#define MX6_PHYFLEX_TLVAIC31_RST    IMX_GPIO_NR(4, 5)
...
static int mx6_phyflex_fec_phy_init(struct phy_device *phydev)
{
	...
	printk("module_rev = %d,%d\n",module_rev, PHYFLEX_MODULE_REV_2);
...
static struct i2c_board_info mxc_i2c1_board_info[] __initdata = {  //VGA I2C
	{
...
static struct i2c_board_info mxc_i2c2_board_info[] __initdata = {   // PAL I2C
	{
/*		I2C_BOARD_INFO("max1037", 0x64),
	} , {
		I2C_BOARD_INFO("stmpe811", 0x41),
...
static struct ipuv3_fb_platform_data phyflex_fb_data[] = {
    {
        .disp_dev		= "ldb",
        .interface_pix_fmt	= IPU_PIX_FMT_RGB24, //IPU_PIX_FMT_RGB666,  //by lwx
        .mode_str		= "LDB-VGA",
        .default_bpp		= 16,
        .int_clk		= false,
    },
...
static struct gpio_keys_button imx6q_buttons[] = {
    GPIO_BUTTON(MX6_PHYFLEX_POWER_OFF, KEY_POWER, 1, "power", 1, 1),
};
...
#endif /*CONFIG_SND_SOC_IMX_TLV320AIC3007 || CONFIG_SND_SOC_IMX_TLV320AIC31 */
	return 0;
}
...
static struct mxc_camera_pdata mxc_ipu_csi_pdata[] = {
	{	
		.flags = MXC_CAMERA_DATAWIDTH_8,  // add by lwx bt656 is 8 bit
		.ipu = 0,
		.csi = 0,
		...
		.flags = MXC_CAMERA_DATAWIDTH_16,
		.ipu = 1, /* changes to 0 in runtame for mx6dl and mx6sl */
		.csi = 1,
		.mclk_default_rate = 108000000,
		.mclk_target_rate =  108000000,	/* for adv7842 vga */
		.use_pll = 0,					
	},
};
...
static struct fsl_mxc_camera_platform_data camera_datas[] = {
    [0]= {
	    .mclk		= 27000000,
	    .mclk_source	= 0,
	    .csi		= 0,
...
//		.platform_data = (void *)&camera_datas[0],   
...
static void init_poweroff_gpio()
{
	gpio_request(MX6_PHYFLEX_POWER_SND_OFF , "poweroff-back");
	gpio_direction_output(MX6_PHYFLEX_POWER_SND_OFF, 1);
    gpio_set_value(MX6_PHYFLEX_POWER_SND_OFF, 0);	
}
...
.name = "soc-camera-pdrv", .id = dev_id, .dev = { .platform_data = &iclinks[dev_id] }
	...
	static struct i2c_board_info phyflex_cameras[] = {
                        [0] = {I2C_BOARD_INFO("adv7842pal", 0x20),},  
                        [1] = {I2C_BOARD_INFO("adv7842vga", 0x20),},
                };
                static struct soc_camera_link phyflex_iclinks[] = {
                        {SOC_CAM_LINK(0, &phyflex_cameras[0], 2)},  //by lwx 7842-PAL ---->i2c2
                        {SOC_CAM_LINK(1, &phyflex_cameras[1], 1)},  // by lwx 7842-vga----->i2c1
                };
                static struct platform_device mxc_ipu_cameras[] = {
		...
		if(csi0_cam_type!=NULL){/* set the max MCLK for the Camera-Type and Interface-Type */
 			if(strcmp(csi0_interface_type,"phyCAM-P")==0) {
                ...
                if(strcmp("adv7842pal",csi0_cam_type)==0) {  
					mxc_ipu_csi_pdata[0].mclk_default_rate = 27000000; // 7842-pal BT656 的时钟频率需要确定
					mxc_ipu_csi_pdata[0].flags = MXC_CAMERA_DATAWIDTH_8; 
				}
		...
		if(csi1_cam_type!=NULL) {/* set the max MCLK for the Camera-Type and Interface-Type */
                ...
                if(strcmp("adv7842vga",csi1_cam_type)==0) {    
					mxc_ipu_csi_pdata[1].mclk_default_rate = 108000000; // 7842-vga 1280*1024@60fps(108MHZ)
					mxc_ipu_csi_pdata[1].flags=MXC_CAMERA_DATAWIDTH_16;
                	}
	...
	/* Registering cameras 移到下面了*/
	i2c_register_board_info(2, camera_i2c2, ARRAY_SIZE(camera_i2c2));  // /dev/video0
	i2c_register_board_info(1, camera_i2c1, ARRAY_SIZE(camera_i2c1));  // /dev/video1
#endif
